...
// --- Firebase Configuration - Updated with the new project credentials ---
const firebaseConfig = {
    apiKey: "AIzaSyDHMAKk7UbRXJczugAzSCmvGANQ0mIZLTc",
    authDomain: "hotleadsapp.firebaseapp.com",
    projectId: "hotleadsapp",
    storageBucket: "hotleadsapp.appspot.com", // Using standard .appspot.com which is more common
...
// Firestore collection paths (use actual UID from Firebase Auth)
...
const Modal = ({ children, onClose, title, maxWidth = "max-w-md" }) => (
...
const InputField = ({ label, name, type = "text", value, onChange, placeholder, children, required = false, step, min, max }) => (
    <div>
...
// --- AddItemModal for Settings ---
const AddItemModal = ({ isOpen, onClose, onSave, listNameLabel }) => {
...
const App = () => {
    ...
    useEffect(() => {
        const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
            setUser(currentUser); 
            setIsAuthReady(true);
             if (currentUser && !sessionStorage.getItem(`welcomeShown_${currentUser.uid}`)) {
                 setToastMessage({text: "Bem-vindo(a) ao HOTLEADS!", type: "success"});
                 sessionStorage.setItem(`welcomeShown_${currentUser.uid}`, 'true');
             } else if (!currentUser) {
                 Object.keys(sessionStorage).forEach(key => {
                    if (key.startsWith('welcomeShown_')) {
    ...
    useEffect(() => {
        ...
        setLoading(true);
        const settingsDocPath = getSettingsPath(user.uid); 
        ...
        const unsubscribeSettings = onSnapshot(settingsDocRef, async (docSnap) => {
            if (docSnap.exists()) {
                ...
                console.log("Settings document does not exist for user, initializing with defaults.");
                try {
                    await setDoc(settingsDocRef, DEFAULT_SETTINGS);
                    setSettings(DEFAULT_SETTINGS); 
                } catch (error) {
                    console.error("Error initializing user settings in Firestore:", error);
                    setSettings(DEFAULT_SETTINGS); 
                }
            }
        }, (error) => {
            ...
            setSettings(DEFAULT_SETTINGS); 
            setToastMessage({text: "Erro ao carregar configurações.", type: "error"})
        });
    ...
    useEffect(() => {
        ...
        const qLeads = query(leadsCollectionRef);
        const unsubscribeLeads = onSnapshot(qLeads, (querySnapshot) => {
            ...
            setToastMessage({text: "Erro ao carregar leads.", type: "error"});
        });
        
        const goalsCollectionRef = collection(db, getGoalsCollectionPath(user.uid));
        const qGoals = query(goalsCollectionRef);
        const unsubscribeGoals = onSnapshot(qGoals, (querySnapshot) => {
            const goalsData = querySnapshot.docs.map(d => ({ id: d.id, ...d.data() }));
            setGoals(goalsData);
            setLoading(false); 
        }, (error) => {
            console.error("Error fetching goals for user:", error);
            setToastMessage({text: "Erro ao carregar metas.", type: "error"});
            setLoading(false); 
        });
    ...
    const updateLead = async (id, updatedData) => {
        if (!user || !user.uid) return;
        try {
            const leadDocRef = doc(db, getLeadsCollectionPath(user.uid), id);
    ...
    const updateGoal = async (id, updatedData) => {
        if (!user || !user.uid) return;
        try {
            const goalDocRef = doc(db, getGoalsCollectionPath(user.uid), id);
    ...
    if (!user) {
        return <AuthView setToastMessage={setToastMessage} />;
    }
